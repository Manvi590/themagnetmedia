          ...
          <div className={`px-8 pb-8 ${footerClassName}`}>
            <div className={`mt-10 flex ${currentStep !== 1 ? 'justify-between' : 'justify-end'}`}>
              {currentStep !== 1 && (
                <button
                  onClick={handleBack}
                  className="duration-350 flex items-center justify-center rounded-full bg-[#bf1b2c] py-1.5 px-3.5 font-medium tracking-tight text-white transition hover:bg-[#981219] active:bg-[#7a0e16]"
                  {...backButtonProps}
                >
                  {backButtonText}
                </button>
    ...
    <motion.div onClick={handleClick} className="relative cursor-pointer outline-none focus:outline-none" animate={status} initial={false}>
      <motion.div
        variants={{
          ...
          active: { scale: 1, backgroundColor: '#bf1b2c', color: '#bf1b2c' },
          complete: { scale: 1, backgroundColor: '#bf1b2c', color: '#bf1b2c' }
        }}
...
function StepConnector({ isComplete }: StepConnectorProps) {
  const lineVariants: Variants = {
    ...
    complete: { width: '100%', backgroundColor: '#bf1b2c' }
  };
